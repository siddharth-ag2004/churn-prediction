...
def fetch_news(api_key, start_date, end_date, keywords, max_articles_per_keyword=30):
    ...
    Returns a list of dictionaries with title, description, date, source, url.
    """
...
def analyze_with_gemini(api_key, articles, peak_dates):
    """
    Sends news and churn peak dates to Gemini API.
    Gemini will match dates and explain possible churn causes, showing article URLs.
    """
...
def main():
    ...
    if not news_api_key or not gemini_api_key:
        print("ERROR: Missing API keys. Check .env for MEDIASTACK_API_KEY and GEMINI_API_KEY.")
        return
    ...
    PEAK_DATES = ["2025-07-08", "2025-07-28", "2025-10-06"]

    print("Fetching news articles from MediaStack...")
    articles = fetch_news(news_api_key, start_date, end_date, KEYWORDS)

    if not articles:
        print("No articles found. Try a more recent date range or check your MediaStack plan.")
        return

    print(f"Fetched {len(articles)} articles. Sending to Gemini for churn insight correlation...")
